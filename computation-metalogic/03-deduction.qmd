...
* **Definition:** A sequent is an expression of the form $ \Gamma \Rightarrow \phi $, where Γ is a set of formulas (premises) and ϕ is a conclusion. This explicitly states that ϕ follows from assumptions Γ.
...
##### 3.3.1 The Type Checker (Proof Verifier)
...
*Notation: This algorithmic view turns the static "Turnstile" ($\vdash$) into an active process. The `verify-proof` function crawls the syntax tree and type-checks each step.*
...
* **Fitch Mechanism:** As shown in the function above, one `open-subproof` with the assumption $P$. If $Q$ is derived within that scope, the subproof is closed and $P \to Q$ is asserted in the main proof line. The vertical bar indicates that $P$ is discharged.
* **Sequent Mechanism ($ \rightarrow I $):** If one has established the sequent $\Gamma, \phi \Rightarrow \psi$ (meaning $\psi$ follows from assumptions $\Gamma$ and $\phi$), one allows the inference of $\Gamma \Rightarrow \phi \to \psi$. This creates a conditional conclusion while removing the dependency on $\phi$ from the premise set.
...
* **Sequent Mechanism ($ \vee E $):** The rule takes three input sequents: $\Gamma \Rightarrow \phi \lor \psi$ (the disjunction), $\Delta_1, \phi \Rightarrow \chi$ (case 1), and $\Delta_2, \psi \Rightarrow \chi$ (case 2). It allows the derivation of $\Gamma, \Delta_1, \Delta_2 \Rightarrow \chi$.
...
<syntax-sandbox title="Interactive Sandbox: Reductio Ad Absurdum" description="Experiment with constructing proofs by contradiction (RAA)." data-groups='[
  { "label": "Sentences", "buttons": ["P", "Q", "R", "S", "ϕ", "ψ", "χ"] },
  { "label": "Context", "buttons": ["Γ", "Δ"] },
  { "label": "Connectives", "buttons": [{"val": "¬", "spaceAfter": false}, "∧", "∨", "→", "↔", "⊥"] },
  { "label": "Turnstiles", "buttons": ["⊢", "⇒"] },
...
* **Fitch Mechanism:** The system uses a **boxed constant**. One opens a subproof declaring a new constant $c$ (boxed at the top). Deriving $P(c)$ within this subproof—where $c$ is isolated from outside premises—allows the export of $\forall x P(x)$.
...
* **Fitch Mechanism:** From a premise $\exists x S(x)$, one opens a subproof assuming $S(c)$ for a *new* name $c$. If one can derive a sentence $Q$ (which does not contain $c$), one can assert $Q$ in the main proof.
...
It is critical to distinguish between **Deductive Strategies**  and **Inductive Strategies**.
* **Deduction :** We use Velleman's heuristics to *enact* the system (deriving formulas). Here, we rarely use Mathematical Induction unless the specific theory (like Peano Arithmetic) includes an induction schema.
* **Metalogic :** We use Velleman's "Structured Approach" to *analyze* the system (proving Soundness/Completeness). Here, Mathematical Induction is the primary engine of proof.
...
<syntax-sandbox problem-id="03-deduction-prob1" title="Problem 1: Rules of Inference" description="Define the axioms and inference rules (e.g., Modus Ponens equivalent) that allow your system to 'compute' proofs. How does one statement legally transition into the next?" data-groups='[
  { "label": "Sentences", "buttons": ["P", "Q", "R", "S", "ϕ", "ψ", "χ"] },
  { "label": "Context", "buttons": ["Γ", "Δ"] },
  { "label": "Connectives", "buttons": [{"val": "¬", "spaceAfter": false}, "∧", "∨", "→", "↔", "⊥"] },
  { "label": "Turnstiles", "buttons": ["⊢", "⇒"] },
...
<syntax-sandbox problem-id="03-deduction-prob2" title="Problem 2: Proof Strategy" description="If your goal in a Fitch proof is to derive P → (Q ∨ R), what should your immediate first step be according to Velleman's goal-directed strategies?" data-groups='[
  { "label": "Sentences", "buttons": ["P", "Q", "R", "S", "ϕ", "ψ", "χ"] },
  { "label": "Context", "buttons": ["Γ", "Δ"] },
  { "label": "Connectives", "buttons": [{"val": "¬", "spaceAfter": false}, "∧", "∨", "→", "↔", "⊥"] },
  { "label": "Turnstiles", "buttons": ["⊢", "⇒"] },
...
[Continue to Part 4: Semantics and Translation →](04-semantics.qmd)
